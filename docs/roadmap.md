...
The next small implementation slice should finish Phase 2:

1. Add generated `.pic` plus user override `.pic` merging.
2. Let overrides patch visibility, fixture inputs, explicit action configuration, and selected labels without changing source-owned declarations by accident.
3. Parse the merged `.pic` back through `parsePieceDslFile()`.
4. Compare merged output against expected package deltas so override behavior is deterministic.
