...
- `compilePieceAction()` is the Node action runner for analyzed packages. It accepts an explicit `actionPackage`, `analysis.actionPackage`, `analysis.snapshot.actionPackage`, a safe selected `analysis.packageScope.packageView`, or the current-file `analysis.piecePackage`, resolves source and language from that analysis, and dispatches to `compileGoPieceFile()` or `compileKotlinPieceFile()` with the selected action identity.
- `compilePieceApp({ compileAction: true })` keeps the normal app status and preview behavior, then attaches the selected language compile report as `status.compileAction`. The option is off by default, so preview-only hosts do not invoke language toolchains accidentally. App-level status also exposes `compileActionSelection`, including action package source, feedback-scope blockers, package-scope candidate/selected state, and source-set scope. If action dispatch cannot select a supported compile action, the status remains available with `compileActionDiagnostics`.
...
- generated Piece action inputs now include target source, dependency, fallback-scope, structured fallback-reason, source-set, Go toolchain and package-source scope, compiler-options, and dependency-artifact hashes; snapshot artifact cache keys include the action-cache identity, and preview runtime cache keys include the fallback-scope identity;
