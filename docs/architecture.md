...
- generated `.pic` files and package targets are metadata first, with target-level `source` only emitted when a target belongs to a different source label than the package default, selected package-view `.pic` output only after the safe package-scope gate passes, and override merging applied against the selected package view only when the Node host passes it explicitly;
