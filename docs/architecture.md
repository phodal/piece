...
- `compilePieceApp({ compileAction: true })` keeps the normal app status and preview behavior, then attaches the selected language compile report as `status.compileAction`. The option is off by default, so preview-only hosts do not invoke language toolchains accidentally. If action dispatch cannot select a supported compile action, the status remains available with `compileActionDiagnostics`.
