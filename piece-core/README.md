...
- `jvmMain`: Kotlin compiler PSI extractor that maps a single `.kt` file into a `PiecePackage`, the Node-callable PSI analysis backend with optional Kotlin compiler semantic diagnostics and local symbol binding, and the Kotlin/JVM compile backend used by the npm host.
