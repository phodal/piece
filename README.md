...
- A Kotlin/JVM PSI analysis backend. The `piece-compiler/node` entrypoint uses it by default for `.kt` and `.kts` files; the root and browser-safe paths keep the lightweight npm extractor. Node callers can opt into Kotlin compiler semantic diagnostics through the same JVM backend.
...
piece-core/
  ...
  src/jvmMain/          Kotlin PSI extraction, compiler diagnostics, and compile backend
...
`npm run language:analysis:smoke` verifies that the Node entrypoint routes Kotlin analysis through the JVM PSI backend and can opt into Kotlin compiler semantic diagnostics. `npm run language:compile:smoke` requires a local Go toolchain. It compiles a real Go single-file module, then asks the Kotlin/JVM backend to compile one Kotlin file for JVM, JS, and Wasm.
