...
- A language-neutral `feedbackScope` explanation that reports whether feedback is handled at piece, file, source-set, or project level. Unknown edges, top-level effects, slice safety flags, and Gradle project-model fallback all become documented reasons. Selected Kotlin Gradle/KMP scopes expose their source roots, classpath, dependency coordinates, project dependencies, target variants, and `source-set:<scopeHash>` action input, while generated action inputs plus snapshot and runtime-cache hashes include source, dependency, and fallback-scope identity.
...
- A Kotlin/JVM analysis backend. The `piece-compiler/node` entrypoint uses PSI by default for `.kt` and `.kts` files; the root and browser-safe paths keep the lightweight npm extractor. Node callers can request `psi`, `fe10-binding-context`, or gated `analysis-api`, and manifests report the requested and actual backend so FE10 fallback is visible. The Analysis API dependency path is opt-in through `pieceAnalysisApiClasspath`; when enabled, the isolated-JVM prototype resolves same-file name references, companion source-set external bindings, imported aliases, simple jar-backed classpath class, constructor, top-level function, extension function, owner-qualified member property symbols, and signature-qualified overload/generic call graph edges through Kotlin Analysis API instead of FE10. The same JVM backend can opt into Kotlin compiler semantic diagnostics; companion files, collected `sourceRoots`, host-provided `classpath` entries, and Gradle/KMP `projectRoot` discovery flow through the same source-set path. Gradle/KMP discovery reports stable project model hashes, resolved module dependency coordinates, resolved project dependencies, target variants, and scope fallback diagnostics; Node narrows analysis to the edited source set plus required shared source sets across the reachable Gradle project dependency closure, or reports why it fell back when the edited file is outside discovered source sets or the selected source set has no matching compile classpath. Generated Piece action inputs plus snapshot cache keys use the scoped model hash. The Kotlin PSI package view now declares compile actions from the Kotlin side, while JS/Wasm bridges only expose that action model to npm/web hosts.
...
`npm run pic:dsl:smoke` verifies that the Node entrypoint routes `.pic` DSL parsing through the JVM ANTLR backend and receives a normal `PiecePackage`. `npm run pic:kotlin:smoke` verifies that Kotlin PSI source extraction emits deterministic `.pic`, reports backend metadata, parses it back through `parsePieceDslFile()`, and gets the same package shape. `npm run pic:source:smoke` verifies that Go and TypeScript extraction expose `analysis.pieceDsl`, parse that `.pic` back through ANTLR, and get the same package shape, including generated action cache inputs. `npm run language:analysis:smoke` verifies that the Node entrypoint routes Kotlin analysis through the JVM backend, reports backend metadata, exposes explicit FE10 selection, shows Analysis API fallback instead of silently claiming support, and can opt into Kotlin compiler semantic diagnostics plus local, companion-file, and `sourceRoots` symbol refinement. `npm run language:analysis-api-gate:smoke` verifies that the optional Analysis API dependency gate is disabled by default and does not resolve external artifacts unless explicitly enabled. `npm run language:analysis-api:smoke` enables that gate and verifies the isolated Analysis API prototype is the actual symbol backend for a shadowed same-file symbol case, a companion source-set external graph edge, an imported alias that keeps the alias as the local graph symbol, a host-provided jar classpath class, jar-backed Kotlin constructors, jar-backed Kotlin top-level plus extension functions, owner-qualified member properties that become external graph edges, callable signatures for overload/generic fixtures, and signature-qualified graph/package dependencies when one declaration calls multiple overloads of the same imported function. It also checks that companion source-set declarations and host-provided external jars are visible to compiler diagnostics instead of being treated as unresolved single-file names. `npm run language:project-model:smoke` creates a real temporary multi-project Gradle/KMP build and verifies that `projectRoot` discovery feeds scoped Kotlin source roots and JVM compile classpaths into Analysis API without hand-supplied `sourceRoots` or `classpath`, that an `:app -> :domain` project dependency enters the edited file's analysis scope, that an unrelated `:unused` project is excluded from that scope, that resolved module coordinates, project dependencies, and target variants are reported, and that unmatched or incomplete project-model scopes expose explicit fallback diagnostics instead of reusing unsafe full-project inputs. It also checks that scoped project model hashes become action/cache-key inputs, then compiles that same saved file through the project's inferred `jvmMain` Gradle variant. `npm run language:compile:smoke` requires a local Go toolchain. It compiles a real Go single-file module, then asks the Kotlin/JVM backend to compile a Kotlin source set with a companion file for JVM, JS, and Wasm.
