...
- Go extraction through a Node-hosted Go AST analyzer, package-local companion graph edges, explicit current-file target policy for companion declarations, candidate package-scope target models with a safe opt-in selection gate, package-scoped `go list -json` action identity, and single-file `go build` / `go test` feedback.
...
- Generated `.pic` metadata and an ANTLR-backed JVM parser for the same package, target, action, artifact, and per-target source model.
